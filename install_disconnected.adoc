...
+
the multicluster engine for Kubernetes operator is included in the {olm} Red Hat Operator catalog.
...
. Now that you have the image in the disconnected {olm}, continue to install the multicluster engine for Kubernetes operator for Kubernetes from the {olm} catalog.
