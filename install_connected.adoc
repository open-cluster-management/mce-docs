...
multicluster engine for Kubernetes, you will need to configure an image pull secret. For information on how to configure an image pull secret and other advanced configurations, see options in the link:./adv_config_install#advanced-config-engine[Advanced configuration] section of this documentation. 
...
. Verify that a 
multicluster engine for Kubernetes engine is not already installed on your OpenShift Container Platform cluster. 
multicluster engine for Kubernetes allows only one single 
multicluster engine for Kubernetes engine installation on each OpenShift Container Platform cluster. Continue with the following steps if there is no 
multicluster engine for Kubernetes engine installed.
...
* Namespace: 
  ...
  - The 
multicluster engine for Kubernetes engine must be installed in its own namespace, or project. 
...
+
After the 
multicluster engine for Kubernetes engine is created, the status for the operator is _Running_ on the _Installed Operators_ page. You can now access the console for the 
multicluster engine for Kubernetes engine. See the following steps:

 . In the OpenShift Container Platform console navigation, select *Networking* > *Routes*.
 . View the URL for your 
multicluster engine for Kubernetes engine in the list, and navigate to it to access the console.
...
multicluster engine for Kubernetes engine on infrastructure nodes, the see link:./install_connected.adoc#infra-olm-sub-add-config[{olm-short} Subscription additional configuration] section.
...
+
*Note:* For installing the 
multicluster engine for Kubernetes engine on infrastructure nodes, see the link:./install_connected.adoc#infra-mce-add-config[MultiClusterEngine custom resource additional configuration] section:
...
After adding infrastructure nodes to your OpenShift Container Platform cluster, follow the link:./install_connected.adoc#installing-from-the-cli[Installing from the OpenShift Container Platform CLI] instructions and add the following configurations to the {olm-short} Subscription and `MultiClusterEngine` custom resource.
