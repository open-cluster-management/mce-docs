...
. The operands can be installed in a designated namespace, by specifying a location in the `MultiClusterEngine` custom resource. This namespace will be created upon application of the `MultiClusterEngine` custom resource.

*Important:* If no target namespace is specified, the namespace of the operator will be used for the operands and will be stated in the `MultiClusterEngine` custom resource spec.

The following example shows the template if you want to specify a target namespace. Replace `target` with the name of your destination namespace
